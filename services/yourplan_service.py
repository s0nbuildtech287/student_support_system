...
def add_subject_to_plan(plan_id, user_id, subject_uid, allow_edit=False):
    ...
    # SIMPLIFIED: Quay lại version đơn giản, dễ debug hơn
    
    # Check plan exists and is_started
    plan = fetch_one(
        "SELECT is_started FROM study_plans WHERE id = %s AND user_id = %s",
        (plan_id, user_id)
    )
    ...
    # Check if plan is started and edit not allowed
    ...
    # Check if subject already exists in plan
    existing = fetch_one(
    ...
    # Insert subject into plan
    success = execute_query(
        "INSERT INTO plan_subjects (plan_id, subject_id, progress, status) VALUES (%s, %s, 0, 'not_started')",
        (plan_id, subject_uid)
    )
...
def remove_subject_from_plan(plan_id, user_id, subject_uid, allow_edit=False):
    ...
    # SIMPLIFIED: Quay lại version đơn giản, dễ debug
    ...
    success = execute_query(
        "DELETE FROM plan_subjects WHERE plan_id = %s AND subject_id = %s",
        (plan_id, subject_uid)
    )
